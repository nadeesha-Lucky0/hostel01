...
export default function StudentApplications({ setAllocatingStudent }) {
    ...
    return (
        <div className="p-4 sm:p-10 space-y-10 w-full animate-fade-in transition-colors">
            ...
            {/* Sub Tabs */}
            <div className="flex gap-1.5 p-1.5 bg-slate-100 dark:bg-slate-800/40 rounded-2xl w-fit mb-8 border border-slate-200 dark:border-slate-700/50">
                <button
                    onClick={() => setActiveSubTab('pending')}
                    className={`px-8 py-3 rounded-xl text-xs font-black uppercase tracking-wider transition-all border-none cursor-pointer flex items-center gap-2 ${activeSubTab === 'pending'
                        ? 'bg-[#FAB95B] text-[#1A3263] shadow-xl shadow-amber-500/20'
                ...
                <button
                    ...
                    className={`px-8 py-3 rounded-xl text-xs font-black uppercase tracking-wider transition-all border-none cursor-pointer flex items-center gap-2 ${activeSubTab === 'allocated'
                        ? 'bg-[#FAB95B] text-[#1A3263] shadow-xl shadow-amber-500/20'
                        : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'
                        }`}
                >
